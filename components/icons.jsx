import React from 'react'
import { makeStyles } from '@material-ui/core/styles'
import SvgIcon from '@material-ui/core/SvgIcon'

const defualtColor = "#d0cfda";

export const MedicalRoomIcon = (props) => (
    <SvgIcon color={defualtColor} stroke={defualtColor} {...props}>
        <g id="medical-room" transform="translate(0.25 0.25)">
            <g id="Group_6791" data-name="Group 6791">
                <g id="Group_6790" data-name="Group 6790">
                    <path id="Path_6644" data-name="Path 6644" d="M18.88,11.205v4.506H17.541a1.958,1.958,0,0,0-1.794-2.743H10.654V1.763A1.765,1.765,0,0,0,8.891,0H1.763A1.765,1.765,0,0,0,0,1.763v18.3H1.175V18.3h8.3v1.763h1.175V16.886H18.88v3.173h1.175V11.205Zm-9.4,5.916h-8.3V4.675a3.28,3.28,0,0,0,2.584-.254,3.185,3.185,0,0,0,3.136,0,3.28,3.28,0,0,0,2.584.254Zm0-13.719A2.086,2.086,0,0,1,7.244,3.25L6.894,2.99l-.351.261a2.025,2.025,0,0,1-2.432,0L3.76,2.99,3.41,3.25A2.086,2.086,0,0,1,1.175,3.4V1.763a.588.588,0,0,1,.588-.588H8.891a.588.588,0,0,1,.588.588Zm6.267,12.309H10.654V14.143h5.092a.784.784,0,0,1,0,1.567Z" stroke-width="0.5" />
                </g>
            </g>
            <g id="Group_6793" data-name="Group 6793" transform="translate(3.203 6.748)">
                <g id="Group_6792" data-name="Group 6792" transform="translate(0 0)">
                    <rect id="Rectangle_405" data-name="Rectangle 405" width="1.053" height="8.603" stroke-width="0.5" />
                </g>
            </g>
            <g id="Group_6795" data-name="Group 6795" transform="translate(6.444 6.748)">
                <g id="Group_6794" data-name="Group 6794" transform="translate(0 0)">
                    <rect id="Rectangle_406" data-name="Rectangle 406" width="0.878" height="8.603"  stroke-width="0.5" />
                </g>
            </g>
        </g>
    </SvgIcon>
)

export const NurseIcon = props => (
    <SvgIcon color={defualtColor} stroke={defualtColor} {...props}>
        <g id="nurse" transform="translate(-81.275 0.25)">
            <path id="Path_6645" data-name="Path 6645" d="M94.129,10.219a5.449,5.449,0,0,0-2.36-1.531,3.688,3.688,0,0,0,.876-2.377V5.128l.213-2.505V2.614A2.382,2.382,0,0,0,92.217.806,2.589,2.589,0,0,0,90.323,0h-3.4a2.59,2.59,0,0,0-1.894.805,2.382,2.382,0,0,0-.641,1.808v.009L84.6,5.128V6.313a3.727,3.727,0,0,0,.84,2.329,5.459,5.459,0,0,0-3.919,5.225v6.084a.6.6,0,0,0,.6.6h0l12.8-.011a.6.6,0,0,0,.6-.6V13.867a5.429,5.429,0,0,0-1.4-3.647ZM86.925,8.4a2.608,2.608,0,0,1-1.116-2.088V5.7h5.632v.608a2.536,2.536,0,0,1-.815,1.839,2.9,2.9,0,0,1-3.7.249ZM85.913,1.627A1.364,1.364,0,0,1,86.926,1.2h3.4a1.364,1.364,0,0,1,1.013.423,1.19,1.19,0,0,1,.321.9L91.49,4.5H85.76l-.168-1.974a1.189,1.189,0,0,1,.321-.9Zm8.415,17.711-11.6.01V13.867a4.249,4.249,0,0,1,3.906-4.226,4.078,4.078,0,0,0,1.989.514h.035a4.094,4.094,0,0,0,1.928-.5,4.233,4.233,0,0,1,3.741,4.21Z" transform="translate(0 0)" stroke-width="0.5" />
            <path id="Path_6646" data-name="Path 6646" d="M283.015,313.243h-.509v-.507a.509.509,0,0,0-1.017,0v.507h-.509a.509.509,0,1,0,0,1.017h.509v.507a.509.509,0,1,0,1.017,0v-.507h.509a.509.509,0,0,0,0-1.017Z" transform="translate(-191.529 -299.279)" stroke-width="0.5" />
            <ellipse id="Ellipse_120" data-name="Ellipse 120" cx="0.509" cy="0.508" rx="0.509" ry="0.508" transform="translate(85.188 13.026)" stroke-width="0.5" />
            <ellipse id="Ellipse_121" data-name="Ellipse 121" cx="0.509" cy="0.508" rx="0.509" ry="0.508" transform="translate(85.188 15.958)" stroke-width="0.5" />
        </g>
    </SvgIcon>
)

export const SettingsIcon = props => (
    <SvgIcon color={defualtColor} stroke={defualtColor} {...props}>
        <g id="settings-cogwheel-button" transform="translate(-3.005 0.75)">
            <g id="settings" transform="translate(3.762)">
                <path id="Path_6819" data-name="Path 6819" d="M19.433,10.058c0-.274.091-.549.091-.914a2.819,2.819,0,0,0-.091-.914l1.92-1.554a.431.431,0,0,0,.091-.549l-1.829-3.2a.512.512,0,0,0-.549-.091l-2.286.914a8.683,8.683,0,0,0-1.554-.914L14.861.457A.368.368,0,0,0,14.5,0H10.838c-.183,0-.457.183-.457.366l-.366,2.469a6.9,6.9,0,0,0-1.554.914L6.266,2.835a.565.565,0,0,0-.64.183L3.8,6.218c-.091.091,0,.366.183.549L5.9,8.229c0,.274-.091.549-.091.914a2.819,2.819,0,0,0,.091.914L3.98,11.612a.431.431,0,0,0-.091.549l1.829,3.2a.512.512,0,0,0,.549.091l2.286-.914a8.683,8.683,0,0,0,1.554.914l.366,2.377a.418.418,0,0,0,.457.366h3.657c.183,0,.457-.183.457-.366l.366-2.377a8.682,8.682,0,0,0,1.554-.914l2.286.914a.44.44,0,0,0,.549-.183l1.829-3.2c.091-.183.091-.457-.091-.549Zm-6.766,2.286a3.2,3.2,0,1,1,3.2-3.2A3.24,3.24,0,0,1,12.667,12.344Z" transform="translate(-3.762)" fill="none" stroke-width="1.5" />
            </g>
        </g>
    </SvgIcon>
)

export const DashboardIcon = props => (
    <SvgIcon {...props}>
        <g id="Group_6788" data-name="Group 6788" transform="translate(0)">
            <path id="Path_6640" data-name="Path 6640" d="M8.788,352H.778a.778.778,0,0,0-.778.778v4.666a.778.778,0,0,0,.778.778h8.01a.778.778,0,0,0,.778-.778v-4.666A.778.778,0,0,0,8.788,352Zm-.778,4.666H1.555v-3.111H8.01Z" transform="translate(0 -338.222)" />
            <path id="Path_6641" data-name="Path 6641" d="M8.788,0H.778A.778.778,0,0,0,0,.778V12.132a.778.778,0,0,0,.778.778h8.01a.778.778,0,0,0,.778-.778V.778A.778.778,0,0,0,8.788,0ZM8.01,11.354H1.555v-9.8H8.01Z" transform="translate(0 0)" />
            <path id="Path_6642" data-name="Path 6642" d="M274.788,0h-8.01A.778.778,0,0,0,266,.778V5.444a.778.778,0,0,0,.778.778h8.01a.778.778,0,0,0,.778-.778V.778A.778.778,0,0,0,274.788,0Zm-.778,4.666h-6.455V1.555h6.455Z" transform="translate(-255.566 0)" />
            <path id="Path_6643" data-name="Path 6643" d="M274.788,180h-8.01a.778.778,0,0,0-.778.778v11.355a.778.778,0,0,0,.778.778h8.01a.778.778,0,0,0,.778-.778V180.778A.778.778,0,0,0,274.788,180Zm-.778,11.354h-6.455v-9.8h6.455Z" transform="translate(-255.566 -172.91)" />
        </g>
    </SvgIcon>
)

export const DevicesIcon = props => (
    <SvgIcon color={defualtColor} stroke={defualtColor} {...props}>
        <g id="Group_7795" data-name="Group 7795" transform="translate(-14 -244)">
            <g id="Group_7094" data-name="Group 7094">
                <g id="Rectangle_609" data-name="Rectangle 609" transform="translate(14 244)" fill="none" stroke-width="1.5">
                    <rect width="20" height="20" rx="4" stroke="none" />
                    <rect x="0.75" y="0.75" width="18.5" height="18.5" rx="3.25" fill="none" />
                </g>
                <g id="Path_7445" data-name="Path 7445" transform="translate(17 249)" fill="none">
                    <path d="M0,0H14V6H0Z" stroke="none" />
                    <path d="M 1.5 1.5 L 1.5 4.5 L 12.5 4.5 L 12.5 1.5 L 1.5 1.5 M 0 0 L 14 0 L 14 6 L 0 6 L 0 0 Z" stroke="none" />
                </g>
                <line id="Line_97" data-name="Line 97" x2="4" transform="translate(22.5 257.5)" stroke-linecap="round" stroke-width="1.5" />
            </g>
        </g>
    </SvgIcon>
)

export const BackIcon = props => (
    <SvgIcon {...props}>
        <path class="a" d="M15.26,6.676H3.624L8.965,1.335,7.63,0,0,7.63l7.63,7.63,1.335-1.335L3.624,8.584H15.26Z"/>
    </SvgIcon>
)